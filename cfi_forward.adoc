...
in bits 31:12 of `x7` is then checked by the `lpad` instruction. The following
example instruction sequence sets up the expected landing pad label as 0xAAB33
in `x7` register prior to the indirect call to function `bar` that is assigned
the label 0xAAB33.
...
of indirect jumps and indirect calls. When forward-edge CFI is active, the
