...
`rs1 != x5` is an indirect jump; it updates the `ELP` to `LP_EXPECTED`. A
`C.JALR` is an indirect call; it updates `ELP` to `LP_EXPECTED`. A `C.JR` where
`rs1 != x1` and `rs1 != x5` is an indirect jump; it updates `ELP` to
`LP_EXPECTED`.
...
if `xFCFIE` is 1 at privilege level. The `xPELP` is set to `NO_LP_EXPECTED`.
Trap handlers should restore the preserved `lpl` value before executing the
`SRET` or `MRET`.
