...
stream is not 4-byte aligned, or is not `lpad`, or if the label part encoded
in `lpad` does not match the expected landing pad label in bits 31:12 of the
`x7` register, then an illegal-instruction exception is raised else the `ELP`
is updated to `NO_LP_EXPECTED`.
...
landing pad label is benign since their use is mutually exclusive.
====
...
[NOTE]
====
A single `lui` instruction may be used to store the expected label value in
...
The following example instruction sequence sets up the expected landing pad
label as 0xAAB33 in `x7` register prior to the indirect call to function `bar`
that is assigned the label 0xAAB33.
...
indirect calls. When forward-edge CFI is active, the instruction causes an
illegal-instruction exception if it is not at a 4-byte aligned `pc`.

The `lpad` has the landing pad label encoded in the `LPL` field. If `LPL` is not
0, `lpad` causes an illegal-instruction exception if the `LPL` does not match
bits 31:12 of `x7` register.
...
is provided in the `mstatus` CSR.

To store the previous `ELP` state on trap delivery to S/HS-mode, a `SPELP`
bit is provided in the `mstatus` CSR. The `SPELP` bit in `mstatus` can be
accessed through the `sstatus` CSR.

To store the previous `ELP` state on traps to VS-mode, a `SPELP` bit is
defined in the `vsstatus` (VS-modes version of `sstatus`).
