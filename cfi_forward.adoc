...
When `henvcfg.CFI` is 0, then Zicfi is not enabled use when `V=1`.
...
The term `xFCFIEN`, used to determine if forward-edge CFI is active, is defined
as follows:

.`xFCFIEN` determination
[source, text]
----
if ( privilege == M-mode ) 
    xFCFIEN = mstatus.MFCFIEN
else if ( menvcfg.CFI == 1 && V == 0 && privilege == S-mode )
    xFCFIEN = mstatus.SFCFIEN
else if ( menvcfg.CFI == 1 && V == 0 && privilege == U-mode )
    xFCFIEN = mstatus.UFCFIEN
else if ( menvcfg.CFI == 1 && henvcfg.CFI == 1 & V == 1 && privilege == S-mode )
    xFCFIEN = vsstatus.SFCFIEN
else if ( menvcfg.CFI == 1 && henvcfg.CFI == 1 & V == 1 && privilege == U-mode )
    xFCFIEN = vsstatus.UFCFIEN
else
    xFCFIEN = 0
----

When forward-edge CFI is not active(`xFCFIEN = 0`):
