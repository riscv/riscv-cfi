...
in the `lpad` does not match the expected landing pad label in bits 31:12
...
embeds a 20-bit immediate value called the landing-pad-label (`LPL`). If
`LPL` is not 0, then the instruction compares the `LPL` to the bits 31:12 of
the `x7` register and a mismatch causes an illegal-instruction exception.

[NOTE]
====
The `lpad` with `LPL` value of 0 does not perform a label check. This enables
...
the `lpad` does not do any label check with `LPL` is 0.

A single `lui` instruction can be used to store the expected label value in
bits 31:12 of the `x7` register at the indirect call/jump site. The label held
in bits 31:12 of `x7` is then checked by the `lpad` instruction.
...
The forward-edge CFI introduces the landing pad - `lpad` - instruction
(See <<LP_INST>>). This instructions is encoded using the `AUIPC` major opcode
with `rd=x0`. If forward-edge CFI is not active (See <<FCIFIACT>>) or if the
Zisslpcfi extension is not implemented, the instruction reverts to being a
no-op.
...
The `lpad` has the landing pad label embedded in the `LPL` field.
If `LPL` is not 0, `lpad` causes an illegal-instruction exception if the
`LPL` field in the instruction does not match bits 31:12 of `x7` register.
...
bytes 3 and 2 have a pattern of `?013h` (for example, the immediate fields of a
`lui`, `auipc`, or a `jal` instruction), followed by a 16-bit or a 32-bit
instruction.
