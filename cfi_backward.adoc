...
5. A leaf PTE has been found. If the memory access is a load from shadow stack
   ...
   a store or an instruction fetch and `pte.xwr == 010b` then cause an 
   access-violation exception corresponding to the original access type. If the
   requested memory access is not allowed by the `pte.r`, `pte.w`, `pte.x`, and
   `pte.u` bits, given the current privilege mode and the value of the `SUM` and
   `MXR` fields of the `mstatus` register, stop and raise a page-fault exception
