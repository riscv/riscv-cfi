...
using the `c.mop.0` and `c.mop.2` encodings defined by the Zimop extension.
Unlike the uncompressed Zimop, the compressed Zimop are defined to not write
`rd`.
...
* The 16-bit instructions defined for backward-edge CFI revert to their Zimop
