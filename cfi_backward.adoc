...
{reg: [
  ...
  {bits:  3, name: 'funct3', attr:['100']},
  {bits:  5, name: 'nzuimm'},
  {bits: 12, name: '100000011101', attr:['ssincp']},
], config:{lanes: 1, hspace:1024}}
....
...
Only non-zero encodings of `nzuimm` are defined for `ssincp`.
...
The `ssincp` instruction adds the zero-extended non-zero immediate `nzuimm`,
...
[listing]
    function_entry:
        ...
        ssincp 1       # increment ssp by 1 * (XLEN/8)
        #
...
The operation of the `ssincp` instruction is as follows:
