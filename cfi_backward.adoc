...
----
If (xBCFIE = 1)
   *[ssp] = [src - (XLEN/8)]   # Store src value to ssp - XLEN/8
...
The `ssp` is incremented by `sspop` only if the load from shadow stack completes
successfully. The `ssp` is decremented by `sspush` only if the store to the
shadow stack completes successfuly.
