...
|`sspush`    2+^|`1000101` 2+^|`src` 2+^|`0`     2+^|`100`    2+^|`0`    2+^|`1110011`
...
1. If `pte.v = 0` or if any bits of encodings that are reserved for future
...
2. Otherwise, the PTE is valid. If `pte.r = 1` or `pte.w = 1` or `pte.x = 1`,
...
3. A leaf PTE has been found. If the memory access is by a shadow stack
