...
|`sspopchkra x1` 2+^|`1000101` 2+^|`00000` 2+^|`00001` 2+^|`100`    2+^|`0`    2+^|`1110011`
|`sspopchkra x5` 2+^|`1000101` 2+^|`00000` 2+^|`00101` 2+^|`100`    2+^|`0`    2+^|`1110011`
...
Only `x1` and `x5` encodings are supported as `rs1` for `sspopchkra`.
Only `x1` and `x5` encodings are supported as `rs2` for `sspush`.

While any register may be used a link register, conventionally the `x1` or `x5`
...
A `sspopchkra` instruction is provided to pop the shadow return address value
...
[literal]
    function_entry:
        ...
        addi sp,sp,8   
        sspopchkra x1  # compare link register x1 to shadow 
...
These examples illustrates the use of `x1` register by the ABI as the link
...
The `sspop` and `sspopchkra` instructions perform a load identically to the
...
The `sspush`, `sspopchkra`, and `sspop` require the virtual address in `ssp` to
have a shadow stack attribute (see <<SSMP>>).

Correct execution of `sspush`, `sspopchkra`, and `sspop` requires that `ssp`
refers to idempotent memory, because the core must be able to handle traps
detected by these instruction (e.g., a page fault) and re-execute them after
returning from the trap handler. If the memory reference by the `ssp` is not
idempotent then the `sspush` instruction causes a store/AMO access fault and the
`sspop`/`sspopchkra` instructions cause a load access fault.

If the virtual address in `ssp` is not `XLEN` aligned then the `sspop`/
`sspopchkra` instructions cause a load address-misaligned exception and the
`sspush` instruction causes store/AMO address-misaligned exception.
...
Locating shadow stacks in non-idempotent memory (e.g., device memory) is not
required and requiring memory referenced by `ssp` to be idempotent does not pose
...
----
if (xBCFIE = 1)
    [ssp] = [ssp] - (XLEN/8)   # decrement ssp by XLEN/8 and
   *[ssp] = [src]              # store src value to address in ssp.
                               # Only x1 or x5 may be used as src
else
    [dst] = 0
endif
----
...
The operation of the `sspopchkra` instruction is as follows:

.`sspopchkra` operation
[source, text]
----
if (xBCFIE = 1)
...
.. `sspop` and `sspopchkra` causes a load access fault
...
`ssamoswap`, and `sspopchkra` to be idempotent.
...
* `sspush`, `sspop`, `sspopchkra`, and `ssamoswap` instructions must match PMP
   entry `sspmp`.
...
inaccessible for U-mode and S-mode read and write accesses. Allowing write access
