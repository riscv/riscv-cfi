...
enables and CFI state for each mode in the `mstatus` CSR. A subset of the fields
...
[NOTE]
====
Tracking CFI state and enables individually for each privilege level enables
applications that use CFI to co-exist with applications do not use CFI; with the
operating system context switching the CFI enables and CFI state of the
...
hypervisor itself. Virtual machines that use CFI may to co-exist with virtual
machines that do not use CFI; with the hypervisor context switching the CFI
enables and state of the virtual machine. Machine mode firmware may enable use
of CFI independent of the use of CFI lower privilege modes.
====
