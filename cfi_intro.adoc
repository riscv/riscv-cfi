...
[NOTE]
====
Compilers should flag each object file (for example, using a flag in the elf
attributes) to indicate if the object file has been compiled with the Zicfisslp
instructions. The linker should flag the binary/executable generated by linking
objects as being compiled with the Zicfisslp extension only if all the objects
files linked have the same Zicfisslp attribute.
...
libraries) used by that process have the Zicfisslp attribute. When the use of
the extension is not enabled for a process then the Zicfisslp instructions in
that application or in the dynamically linked libraries invoked by that
process revert to their Zimop/Zccmop defined behavior. This allows the program
...
In case a program compiled with the Zicfisslp extension is installed on an
operating system that is not modified to support the extension or on a machine
that does not support it, the Zicfisslp instructions will revert to their
Zimop/Zcmop defined behavior. This allows an application developer to distribute
a single application that can be used on machines and/or OS installations with
