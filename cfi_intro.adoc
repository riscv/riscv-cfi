...
Zicfisslp extension provides a for Control-flow Integrity (CFI) capabilities to
defend against Return-Oriented Programming (ROP) and Call/Jump-Oriented
Programming (COP/JOP) style control-flow subversion attacks. This attack
methodology uses code sequences in authorized modules, with at least one
instruction in the sequence being a control transfer instruction that depends on
attacker-controlled data either in the return stack or in a register/memory for
the target address. Attackers stitch these sequences together by diverting the
control flow instruction (e.g., `JALR`, `C.JR`, `C.JALR`), from its original
target address to a new target via modification in the return stack or in the
register or memory used by the instruction.
...
a landing pad instruction that allows software to indicate valid targets for
...
The landing pad is designed to provide integrity to control transfers
...
The landing pad may be labeled. With labeling enabled, the number of landing
...
fields in the `mstatus` CSR are accessible using the `sstatus` CSR. VS-mode’s
...
instructions revert to their Zimop/Zcmop defined behavior when the Zicfisslp
...
the extension is not implemented, the landing pad instruction executes as a
no-op.
...
The dynamic loader should activate the use of Zicfisslp extension for a process
...
to operate correctly but without control-flow integrity.

A process that has the Zicfisslp extension active may request the dynamic
...
deactivate the extension for the process. If the extension is disabled then the
Zicfisslp instructions revert to their Zimop/Zcmop defined behavior and the
program continues to operate correctly but without control-flow integrity.
...
programs are executed in user mode, the OS may deactivate the Zicfisslp
extension for that process. When the Zicfisslp instructions in the system
libraries are invoked by these programs in user mode, they revert to their
Zimop/Zcmop defined behavior. The OS only needs to carry one version of the
system libraries, which are usable by both applications that use the extension
...
and those that do not. The OS continues to operate correctly but without
control-flow integrity on machines that do not support the extension.
...
support for the Zicfisslp extension and those that do not. The application
continues to operate correctly but without control-flow integrity when the
...
instructions, the program may continue to operate correctly. On other hand
the use of landing pads requires that all binaries used by a program must all be
compiled with the landing pad instructions. Due to each binary needing to be
compiled to include the landing pad instruction the use of the `AUIPC` HINT code
points for landing pads does not lead to compatibility issue. Due to this only
the shadow stack instructions are encoded using the Zimop/Zcmop defined code
points. The use of the `AUIPC` HINT code points for landing pad enables compact
encoding of the landing pad label label in the instruction.
