...
support the Zisslpcfi extension as well as those that do not. Most programs are
...
do not support the Zisslpcfi extension. Some just-in-time (JIT) compiled
...
To enforce forward edge control-flow integrity, Zisslpcfi extension introduces
...
[NOTE]
====
The operating system may activate the use of Zisslpcfi by U-mode applications,
...
compiled with Zisslpcfi capabilities and others that are not. The operating
system can activate or deactivate the use of the extension in U-mode per
application by context switching the Zisslpcfi state.

Hypervisors may activate the use of Zisslpcfi in a virtual machine, with or
...
hypervisor context switching the Zisslpcfi state of each virtual machine.

To use Zisslpcfi, the operating system and hypervisors have to be modified to
...
The Zisslpcfi instructions are encoded using a subset of "May be op"
instructions defined by the Zimop and Zcmop extensions. This subset of
instructions reverts to their Zimop/Zcmop defined behavior when the Zisslpcfi
extension is not implemented or if the extension has not been activated at a
privilege mode. A program that is built with Zisslpcfi instructions can thus
...
[NOTE]
====
An OS modified to support the Zisslpcfi extension typically includes system
libraries (such as glibc) that are also compiled with the Zisslpcfi extension.
However, these system libraries may need to dynamically link to programs that
are not compiled with the Zisslpcfi extension. In such cases, when these
programs are executed in user mode, the OS may disable the Zisslpcfi extension.
When the Zisslpcfi instructions in the system libraries are invoked by these
...
An OS distribution compiled with Zisslpcfi extension may be installed on a
machine that does not support the extension. In such cases, the Zisslpcfi
instructions revert to their Zimop/Zcmop defined behavior.

In case a program compiled with the Zisslpcfi extension is installed on an
operating system that is not modified to support the extension or on a machine
that does not support it, the Zisslpcfi instructions will revert to their
Zimop/Zcmop defined behavior.
====

The Zisslpcfi extension depends on the Zicsr, A, Zimop, and Zcmop extensions.
